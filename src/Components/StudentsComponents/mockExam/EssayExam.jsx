...
export default function EssayExam({ goBack }) {
  ...
  const [questions, setQuestions] = useState([]); // [{ id, type: "essay", question }]
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      if (!res.ok) throw new Error((await res.text()) || "Failed to generate essay questions");
      const data = await res.json();

      const qs = (Array.isArray(data.questions) ? data.questions : [])
        .filter(q => q && (q.type === "essay" || !q.type)) // accept if backend didn’t set type
        .slice(0, 10)
        .map((q, idx) => ({
          id: q.id ?? `essay-${idx}`,
          type: "essay",
      ...
      if (qs.length === 0) throw new Error("No essay questions returned from AI");
  ...
  async function handleSubmit() {
    ...
    // OPTIONAL: Save to Supabase (uncomment if you add exam_type column)
    // try {
    //   const payload = {
    //     user_id: uid,
    ...
    //     incorrect: null,     // not auto-graded
    //     unanswered: questions.filter(q => !answers[q.id])?.length ?? 0,
    //     percentage: null,    // not auto-graded
    //     duration_minutes: null,
    //     answers,             // essays as text
    //     exam_type: "essay",  // <<< add this column to table if you want
    //   };
    //   const { error: insertError } = await supabase.from("mock_exam_results").insert([payload]);
    //   if (insertError) console.error("Error saving essay attempt:", insertError.message);
    // } catch (e) {
    //   console.error("Unexpected error saving essay attempt:", e);
    // }
  }
  ...
  return (
    <div className="container py-3">
      ...
      <h2 className="mb-3">Essay Exam</h2>
      <p className="text-muted">
        Generate open-ended questions. Students type short/long answers.
      </p>
      ...
      <div className="row g-2 align-items-end mb-3">
        ...
        <div className="col-12 col-md-auto">
          ...
          >
            {loadingGenerate ? "Generating…" : "Generate Essay Questions"}
          </button>
        </div>
      </div>
      ...
      {!loadingCourses && !loadingGenerate && questions.length === 0 && (
        <div className="text-muted">
          {courses.length === 0
            ...
            : "Pick a course and click Generate Essay Questions."}
        </div>
      )}
    </div>
  );
